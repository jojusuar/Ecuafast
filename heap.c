...
Heap *newHeap(bool isMax) {
    ...
    Heap *heap = (Heap *)mmap(NULL, sizeof(Heap), PROT_READ | PROT_WRITE,
                              MAP_SHARED, structfd, 0);
    close(structfd);
    if (cleanstruct) {
        heap->size = 0;
    ...
    heap->data = (double *)mmap(NULL, heap->max_capacity * sizeof(double),
                                PROT_READ | PROT_WRITE, MAP_SHARED, datafd, 0);
    close(datafd);
    return heap;
}
...
bool isFull(Heap *heap) { return heap->size == heap->max_capacity; }
...
void bubbleUp(Heap *heap, int index) {
    ...
    while (index > 0 &&
           ((!heap->isMax && ((heap->data[index] < heap->data[parent])) ||
             (heap->isMax && (heap->data[index] > heap->data[parent]))))) {
    ...
    while (1) {
        ...
        if (heap->isMax) {
            ...
            if (leftChild < heap->size &&
                (heap->data[leftChild] > heap->data[biggest])) {
                biggest = leftChild;
            }
            if (rightChild < heap->size &&
                (heap->data[rightChild] > heap->data[biggest])) {
                biggest = rightChild;
            }
            if (biggest != index) {
            ...
            smallest = index;
            if (leftChild < heap->size &&
                (heap->data[leftChild] < heap->data[smallest])) {
                smallest = leftChild;
            }
            if (rightChild < heap->size &&
                (heap->data[rightChild] < heap->data[smallest])) {
                smallest = rightChild;
            }
            if (smallest != index) {
